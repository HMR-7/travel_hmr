<template>
	<view class="content" v-if="goodDetail">
		<van-popup :show="closeCanvas" @close="toCloseCanvas" custom-style="margin:0;background-color:rgba(0,0,0,0);">
			<view id="canvas-container" v-if="closeCanvas">
				<canvas canvas-id="myCanvas" style="width:700rpx; height:450rpx"></canvas>
			</view>
			<button v-show="posterPath" style="margin-top:20rpx;background-color: var(--themeColor);color: #fff;" @tap="saveImg">保存</button>
		</van-popup>
		<!-- 轮播图 -->
		<swiper class="img-container" indicator-dots="true" indicator-color="var(--detailColor)" indicator-active-color="var(--themeColor)"
		 circular autoplay>
			<block v-for="(item, index) in imgArr" :key="index">
				<swiper-item class="item">
					<!-- <view class="swiper_title">{{item.title}}</view> -->
					<image class="itemImg" :class="{active:currentIndex==index}" :src="item" @tap="previewImage(imgArr,item,index)" />
				</swiper-item>
			</block>
		</swiper>
		<!-- 景点简介部分 -->
		<view class="megs_box mar_bottom">
			<view class="header">
				...
				<!-- 商品收藏 -->
				<view :class="isCollect ==1?' icon-guanzhu1 icon_select_color':'icon-guanzhu2'" class="iconfont" style="font-size:40rpx;"
				 @tap="handleCollect"></view>
			</view>
			<view class="middle">
			...
			<view class="bottom">
				<view class="place mar_bottom" v-if="goodDetail.address==null" style="
        color: #7a7e83;
        ">地址：暂无地址信息</view>
				<view class="place mar_bottom" v-if="goodDetail.address!=null" style="
        width: 500rpx;
        ...
        ">地址：{{goodDetail.address}}</view>
				<view class="place mar_bottom" v-if="goodDetail.good_price==undefined" style="color:var(--priceColor)">门票价：--元/起</view>
				<view class="place mar_bottom" v-if="goodDetail.good_price!=undefined" style="color:var(--priceColor)">门票价：{{goodDetail.good_price}}元/起</view>
			</view>
		</view>
		<!-- 门票信息-使用自定义滑动组件 -->
      ...
      <view class="TicketMegs">
		...
		<view v-if="!log">
			...
			<view class="TicketMegs">
				...
				<view class="TicketPrice">
					<view class="PriceTitle">
						成人票
						<view v-if="goodDetail.good_price==undefined" style="font-size:24rpx;font-weight:bolder;color: var(--priceColor);">￥--元/起</view>
						<view v-if="goodDetail.good_price!=undefined" style="font-size:24rpx;font-weight:bolder;color: var(--priceColor); ">￥{{goodDetail.good_price}}元/起</view>
					</view>
				</view>
				<view class="TicketPrice">
					<view class="PriceTitle">
						儿童票
						<view v-if="goodDetail.childTicket==undefined" style="font-size:24rpx;font-weight:bolder;color: var(--priceColor);">￥--元/起</view>
						<view v-if="goodDetail.childTicket!=undefined" style="font-size:24rpx;font-weight:bolder;color: var(--priceColor);">￥{{goodDetail.childTicket}}元/起</view>
					</view>
				</view>
				<!-- 详情咨询提示 -->
				<view class="selMorePrice">详情可到景区售票处询问</view>
			...
			<!-- 美食推荐列表 -->
			<view style="margin:20rpx;border-radius: 20rpx;overflow: hidden;box-shadow: 0 0 20px rgba(0, 0, 0, 0.2);">
			...
			<view class="TicketMegs">
				...
				<view class="TicketPrice" v-for="(item,index) in hotelList" :key="index">
					<view class="PriceTitle" @tap="hotelDetail(item.id)">
						...
						<view v-if="item.hotel_price==null" style="ffont-size:24rpx;font-weight:bolder;color: var(--priceColor);">{{item.hotel_price}}</view>
						<view v-if="item.hotel_price!=null" style="font-size:24rpx;font-weight:bolder;color: var(--priceColor);">¥{{item.hotel_price}}元/起</view>
						<!--   <view
			...
			<van-popup :show="showHotelDetail" @close="showHotelDetail=false" @catchtouchmove="move">
				<view class="handlehotelMegs" :key="index">
					<swiper class="img-container" indicator-dots="true" indicator-color="var(--detailColor)" indicator-active-color="var(--themeColor)"
					 circular autoplay>
						<block v-for="(item1, index1) in hotelDetailMegs.src" :key="index1">
							<swiper-item class="item">
								<image class="itemImg" :class="{active:currentIndex==index}" :src="item1" @tap="previewImage(hotelDetailMegs.src,item1,index)" />
							</swiper-item>
						</block>
					</swiper>
					...
					<view class="bottom">
						...
						<view class="hotelPrice" style="color:var(--priceColor)">¥{{hotelDetailMegs.hotel_price}}元/起</view>
					</view>
		...
		<view class="share">
			...
			<view @click="createPoster" class="moudle" style="background-color: var(--themeColor);color: #fff;border:none;">生成海报</view>
		</view>
		<!-- 用户日志发布区域 -->
		<view @tap="writeLog=true" class="iconfont icon-tianxie" style="color:var(--themeColor);font-size:80rpx;position: fixed;top: 80%;left: 86%;background-color: var(--contentBgc);border-radius: 100%;background: rgba(0,0,0,0);}"></view>
		<van-popup :show="writeLog" position="bottom" @close="writeLog=false">
			<view class="travellog" v-if="writeLog">
				...
				<view style="padding:0 20rpx">
					<textarea class="sugs" cursor-spacing="100" placeholder-style="padding-left:20rpx" placeholder="请输入内容" maxlength="-1"
...
<script>
	...
	export default {
		...
		onLoad(options) {
			...
			console.log(options);
			if (!options.log) {} else if (options.log == 1) {
				t.log = options.log;
		...
		methods: {
			...
			handleCollect() {
				...
				t.$utils.ajax(t.$api.userCollect, "post", data, res => {
					...
					if (t.isCollect == 1) {} else if (t.isCollect == 0) {}
				});
			},
			/* 轮播图点击预览 */
